...
//var _pdf_find_controller = PDFFindController;
...
var PDFViewerApplication = {
  ...
  customLoadResourceFile: function customLoadResourceFile(fileData, isFromBuffers)  //THANGHM 22.02
  {
    if(isFromBuffers)
        PDFViewerApplication.open(new Uint8Array(fileData));
    else
        PDFViewerApplication.open(fileData);
  },
  customWebViewerSearchPDFPosition: function(options)  //THANGHM 22.02
  {
    webViewerSearchPDFPosition(options);
  }
};
var validateFileURL = void 0;
{
  ...
  validateFileURL = function validateFileURL(file) {
    ...
    try {
      if(HOSTED_VIEWER_ORIGINS.length <= 0) {   //THANGHM 13.03.2020
        return;
      }
...
function webViewerSearchPDFPosition(evt)
{
    PDFViewerApplication.findController.executeCommandFindBeginEnd('findBeginEnd', {
        query: evt.query,
        phraseSearch: evt.phraseSearch,
        caseSensitive: evt.caseSensitive,
        highlightAll: evt.highlightAll,
        ...
        typeLocation: evt.typeLocation,
        entireWord: true,
        showToolBar: evt.showToolBar
      });

    PDFViewerApplication.findBar.open();
    PDFViewerApplication.findBar.findField.value = evt.query;
    PDFViewerApplication.findBar.caseSensitive.checked = evt.caseSensitive;
    PDFViewerApplication.findBar.highlightAll.checked =  evt.highlightAll;
}
...
var PDFFindController = function () {
  function PDFFindController(_ref) {
    ...
    this._previousHightlightHasData = false;
  }
  ...
  _createClass(PDFFindController, [{
    ...
    key: 'executeCommandFindBeginEnd',
    value: function executeCommandFindBeginEnd(cmd, state) { //HANT-22.02
        var _this = this;
        var pdfDocument = this._pdfDocument;
        ...
        this._firstPageCapability.promise.then(function () {
            ...
            } else if (cmd === 'findBeginEnd') {
                _this._findTimeout = setTimeout(function () {
                    _this._nextMatchBeginEnd();
                    _this._findTimeout = null;
                }, FIND_TIMEOUT);
            } else {
               _this._nextMatch();
    ...
    key: '_nextMatchBeginEnd',
    value: function _nextMatchBeginEnd() {
        var _this3 = this;
        ...
        this._findFoundBeginEnd = false;

        //HANT-22.02
        var totalWord = 0;
        var beginWord = this._state.begin;
        var endWord = this._state.end;
        var typeLocation = this._state.typeLocation;
        var positionInPage = null;
        var curOfset = null;
        ...
        if (this._dirtyMatch) {
          ...
          this._pageStartBegin =null;
          this._positionInPageStartBegin = null;
        ...
        if(typeLocation)
        {
            var pageHasQuery = 0;
            for (var i = 0; i < numPages; i++) {
                var originPageContent = this._pageContents[i];
                if(originPageContent !== null && originPageContent !== undefined)
                {
                    var pageContent = this._normalize(originPageContent);
                    totalWord = totalWord+ pageContent.length;
                    if( beginWord < totalWord )
                    {
                        positionInPage = beginWord + pageContent.length - totalWord;
                        var query = this._normalize(this._state.query);
                        var queryLen = query.length;
                        var matchIdx = -queryLen;
                        var preMatchIdx = null;

                        var t = 0;
                        while (true) {
                            console.log('HANT t = ',t);
                            preMatchIdx = matchIdx;
                            matchIdx = pageContent.indexOf(query, matchIdx + queryLen);
                            console.log('HANT matchIdx = ', matchIdx);
                            this._offset.pageIdx = i;
                            this._selected.pageIdx = i;

                            if(matchIdx >= positionInPage && preMatchIdx <= positionInPage){
                                this._offset.matchIdx = t;
                                this._selected.matchIdx = t;
                                this._findFoundBeginEnd = true;
                                break;
                            }
                            else if(matchIdx >= 0 && matchIdx < positionInPage && preMatchIdx <= positionInPage)
                            {
                                this._offset.matchIdx = t;
                                this._selected.matchIdx = t;
                                this._findFoundBeginEnd = true;
    ...
    key: 'executeCommandHightLight',
    value: function executeCommandHightLight(cmd, state) {
        var _this = this;
      ...
      key: 'showHightLight',
      value: function showHightLight(state) {
         var numPages = this._linkService.pagesCount;
         //Clear previous hight light
         if(this._previousHightlightHasData) {
             for (var i = 0; i < numPages; i++) {
                var pageView = PDFViewerApplication.pdfViewer.getPageView(i);
                if (pageView.textLayer) {
                    pageView.textLayer.resetHighlightStartEnd();
                }
             }
         }

         var listOffsets = state.listOffs;
         for (var k = 0; k < listOffsets.length; k++) {
             var beginWord = listOffsets[k].posStart;
             var endWord = listOffsets[k].posEnd;
             var startPageIdx = -1;
             var endPageIdx = -1;

             var totalWord = 0;
             var startPositionInPage = 0;
             var endPositionInPage = 0;
             for (var i = 0; i < numPages; i++) {
                var pageContent = this._normalize(this._pageContents[i]);
                totalWord = totalWord + pageContent.length;
                if( beginWord < totalWord && startPositionInPage === 0) {
                    startPageIdx = i;
                    startPositionInPage = beginWord + pageContent.length - totalWord;
                }
                if( endWord < totalWord) {
                    endPageIdx = i;
                    endPositionInPage = endWord + pageContent.length - totalWord;
                    break;
                }
             }

             console.log('THANGHM: 1.0', startPageIdx, endPageIdx);
             var matches = [];
             if(startPageIdx < endPageIdx) {
                for (var index = startPageIdx; index < (endPageIdx + 1); index++) {
                    matches = [];
                    var pageContent = this._normalize(this._pageContents[i]);
                    var pageView = PDFViewerApplication.pdfViewer.getPageView(index);
                    if (pageView.textLayer) {
                        var queryLen = pageContent.length - startPositionInPage;
                        if(index != startPageIdx) {
                            matches.push(0);
                            if(index === endPageIdx) {
                                queryLen = endPositionInPage;
                            }
                            else {
                                queryLen = pageContent.length;
                            }
                        } else {
                            matches.push(startPositionInPage);
                        }

                        var highlightPage = {
                            matches: matches,
                            queryLen: queryLen,
                            selectCssClass: listOffsets[k].posCssClass
                        }
                        pageView.textLayer._highlightStartEnd(highlightPage);
                        this._previousHightlightHasData = true;
                    }
                }
             } else {
                if(startPageIdx !== -1 && endPageIdx !== -1) {
                    matches.push(startPositionInPage);
                    var highlightPage = {
                        matches: matches,
                        queryLen: endPositionInPage - startPositionInPage,
                        selectCssClass: listOffsets[k].posCssClass
                    }
                    var pageView = PDFViewerApplication.pdfViewer.getPageView(startPageIdx);
                    if (pageView.textLayer) {
                        pageView.textLayer._highlightStartEnd(highlightPage);
                        this._previousHightlightHasData = true;
                    }
    ...
    value: function _matchesReady(matches) {
      ...
      var previous = this._state.findPrevious;
      var findFoundBeginEnd = this._findFoundBeginEnd;  //HANT-22.02
      if (numMatches) {
        if(!findFoundBeginEnd) {
    ...
    value: function _nextPageMatch() {
      ...
      do {
        ...
        var matches = this._pageMatches[pageIdx];
        if (!matches){
          this._resumePageIdx = pageIdx;
          break;
        }
        else if(matches.length == 0 && findFoundBeginEnd) //HANT-22.02
        {
            this._updateMatch(true);
            break;
        }
...
var MATCH_SCROLL_OFFSET_TOP = -((screen.height/2) - 100);
...
var TextLayerBuilder = function () {
  ...
  _createClass(TextLayerBuilder, [{
    ...
    key: 'convertMatches',
    value: function convertMatches(matches, matchesLength, queryLenExt) {
      var i = 0;
      var iIndex = 0;
      var textContentItemsStr = this.textContentItemsStr;
      var end = textContentItemsStr.length - 1;
      var queryLen = (queryLenExt !== undefined) ? queryLenExt      //HANT 25.02.2019
                                               : this.findController === null ? 0 : this.findController.state.query.length;
      var ret = [];
      if (!matches || !textContentItemsStr || textContentItemsStr.length <= 0) {
        return ret;
      }
  ...
  {
    key: '_highlightStartEnd',
    value: function _highlightStartEnd(highlightOffset) {
        var resultMatches = this.convertMatches(highlightOffset.matches, null, highlightOffset.queryLen);

        var textContentItemsStr = this.textContentItemsStr;
        var textDivs = this.textDivs;
        var highlightSuffix = ' selected';
         if(highlightOffset.selectCssClass !== undefined) {
            highlightSuffix = ' ' + highlightOffset.selectCssClass;
         }
        ...
        function appendTextToDiv(divIdx, fromOffset, toOffset, className) {
            var div = textDivs[divIdx];
            var content = (toOffset == undefined) ? textContentItemsStr[divIdx].substring(fromOffset)
        ...
        {
            var begin = resultMatches[0].begin;
            var end = resultMatches[0].end;

            if (begin.divIdx === end.divIdx) {
                beginText(end, 'highlight end' + highlightSuffix);
            } else {
                for (var n0 = begin.divIdx, n1 = end.divIdx; n0 < n1 + 1; n0++) {
                    textDivs[n0].className = 'highlight middle' + highlightSuffix;
                }
            }
      ...
      key: 'resetHighlightStartEnd',
      value: function resetHighlightStartEnd() {
        if (!this.renderingDone) {
          return;
        }
        var textDivs = this.textDivs;
        for (var i = 0, len = textDivs.length; i < len; i++) {
          var clearDiv = textDivs[i];
          clearDiv.className = '';
        }
      }
  }]);
